 ...
 * 
 * Generic crontab dialog for scheduled tasks.
 *
 * Usage:
 *		CrontabDialog crontabDialog = new CrontabDialog(Display.getDefault().getActiveShell());
...
//	private static final String HELP = Messages_NLS.CrontabDialog_HELP;
	...
	private final String[] CRON_SHORTCUT_LIST = {Messages_NLS.CrontabDialog_CUSTOM, Messages_NLS.CrontabDialog_YEARLY, Messages_NLS.CrontabDialog_MONTHLY, Messages_NLS.CrontabDialog_WEEKLY, Messages_NLS.CrontabDialog_DAILY, Messages_NLS.CrontabDialog_HOURLY, Messages_NLS.CrontabDialog_ONCE};
	...
	public void create() {
		super.create();
		setTitle(Messages_NLS.CrontabDialog_SCHEDULING_ARGUMENTS);
		setMessage(Messages_NLS.CrontabDialog_PROVIDE_CRONTAB_EXECUTION_PARAMETERS, IMessageProvider.INFORMATION);
	}
...
//		data.heightHint = 300;
	...
	private void createShortcuts(final Composite parent) {
		...
		lblCronShortcuts.setText(Messages_NLS.CrontabDialog_SCHEDULING);
		...
		cmbVwCronSelection.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				// yearly
				case 1: clearAll(); setEnabled( true ); dateTimeChanged = false;  dateTime.setEnabled(false);setValues(new String[]{"0", "0", "1", "1", "?"}); break; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				// monthly
				case 2: clearAll(); setEnabled( true ); dateTimeChanged = false;  dateTime.setEnabled(false);setValues(new String[]{"0", "0", "1", "*", "?"}); break; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				// weekly
				...
				// daily
				case 4: clearAll(); setEnabled( true ); dateTimeChanged = false;  dateTime.setEnabled(false);setValues(new String[]{"0", "0", "*", "*", "?"}); break; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				// hourly
				case 5: clearAll(); setEnabled( true ); dateTimeChanged = false; dateTime.setEnabled(false); setValues(new String[]{"0", "*", "*", "*", "?"}); break; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				// once
				case 6: 
					{
	...
	private void createMinute(Composite parent) {
		...
		lblMinute.setText(Messages_NLS.CrontabDialog_MINUTE);
...
//				}
	...
	private void createDateTime(Composite parent) {
		Label lblMinute = new Label(parent, SWT.NONE);
		lblMinute.setText(Messages_NLS.CrontabDialog_DATE);
...
//				}
	...
	private void createHour(Composite parent) {
		...
		lblHour.setText(Messages_NLS.CrontabDialog_HOUR);
...
//				}
	...
	private void createDayOfMonth(Composite parent) {
		...
		lblDayOfMonth.setText(Messages_NLS.CrontabDialog_DAY_OF_MONTH);
...
//				}
	...
	private void createMonth(Composite parent) {
		...
		lblMonth.setText(Messages_NLS.CrontabDialog_MONTH);
...
//				}
	...
	private void createDayOfWeek(Composite parent) {
		...
		lblDayOfWeek.setText(Messages_NLS.CrontabDialog_DAY_OF_WEEK);
